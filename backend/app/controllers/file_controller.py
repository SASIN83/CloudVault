...
def trash(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def upload_file(parent_id: int, file: UploadFile,
...
def move(item_id: int, payload: MoveRequest,
         db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def copy(item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return file_service.copy_item(db, item_id, user)


@router.delete("/{item_id}", status_code=204)
def to_trash(item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    file_service.soft_delete(db, item_id, user)


@router.post("/{item_id}/restore", status_code=204)
def restore(item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    file_service.restore(db, item_id, user)


@router.delete("/{item_id}/permanent", status_code=204)
def delete_forever(item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def unshare(item_id: int, email: str,
            db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def download(item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
